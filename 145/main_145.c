...
int main(){
	...
	int *loc_rev = (int *)malloc(sizeof(int) * RANGE);

	//Allocate OpenCL memory for results
	cl_mem rev = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
			sizeof(cl_int) * RANGE, NULL, &error);
	errchk(error, "clCreateBuffer");

	FILE *src = fopen("kernel_145.cl", "r");
	...
	cl_kernel rev_kernel = clCreateKernel(program, "rev_count", &error);
	errchk(error, "clCreateKernel");

	error = clSetKernelArg(rev_kernel, 0, sizeof(cl_mem), &rev);
	errchk(error, "clSetKernelArg 0");
	const int rng = RANGE;
	error = clSetKernelArg(rev_kernel, 1, sizeof(cl_int), &rng);
	...
	while (global_ws < RANGE)
		global_ws += local_ws;
	error = clEnqueueNDRangeKernel(queue, rev_kernel, 1, NULL,
					&global_ws, &local_ws, 0, NULL, NULL);
	errchk(error, "clEnqueueNDRangeKernel");

	clEnqueueReadBuffer(queue, rev, CL_TRUE, 0, sizeof(cl_int) * RANGE,
				loc_rev, 0, NULL, NULL);
	...
	free(loc_rev);
	clReleaseKernel(rev_kernel);
	clReleaseCommandQueue(queue);
	clReleaseContext(context);
	clReleaseMemObject(rev);
}
